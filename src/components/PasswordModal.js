...
const PasswordModal = ({ isOpen, onClose, onPasswordSubmit, fileName }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Check if password is correct (this is just for animation)
    // The actual verification happens in the parent component
    if (password === 'rodnan') {
      setShowSuccess(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
      <div 
        ref={modalRef}
        className={`bg-white dark:bg-dark-card rounded-lg shadow-lg max-w-md w-full p-6 transform transition-all duration-500 ${
      ...
      >
        <div className="text-center mb-4 relative">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Protected Download</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Enter the password to download <span className="font-medium">{fileName}</span>
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between mt-6">
            ...
            <button
              ...
              disabled={showSuccess}
            >
              {showSuccess ? 'Downloading...' : 'Download'}
            </button>
          </div>
        </form>
      </div>
